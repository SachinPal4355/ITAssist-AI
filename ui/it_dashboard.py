...
def _render_full_ticket_detail_view(ticket_id: str):
    ...
    status_icons = {"In Progress": "⚙️", "Resolved": "✅", "Escalated": "🚨", "Cancelled": "❌"}
    ...
    if st.button("💾 Save Notes", key=f"save_notes_{ticket_id}"):
...
def _render_overview():
    ...
    cols = st.columns(4)
    with cols[0]:
        render_metric_card("Total Tickets", stats["total"], "🎫", "#6366f1")
    with cols[1]:
        render_metric_card("Open", stats["open"], "🔓", "#3b82f6")
    with cols[2]:
        render_metric_card("In Progress", stats["in_progress"], "⚙️", "#f59e0b")
    with cols[3]:
        render_metric_card("Resolved", stats["resolved"], "✅", "#22c55e")
...
def _render_all_tickets():
    ...
    if not tickets:
        render_info_banner("No Tickets Found", "No tickets match your filter criteria.", icon="🔍")
        return
...
def _render_ticket_detail():
    ...
    all_tickets = get_all_tickets()
    if not all_tickets:
        render_info_banner("No Tickets", "No tickets have been created yet.", icon="📭")
        return
...
def _render_knowledge_base():
    ...
    if not articles:
        render_info_banner("No Articles Found", "Run the app once to seed the knowledge articles.", icon="📭")
    else:
        for art in articles:
